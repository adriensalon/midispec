...
namespace yamaha_dx7 {
    ...
    struct yamaha_dx7 : public hardware {
        ...
        void save_to_voice(const integral<std::uint8_t, 0, 15> device, const integral<std::uint8_t, 0, 31> voice)
        {
            ...
            send(_sent);
            encode_button_voice(_sent, device, voice, true);
            send(_sent);
            encode_button_voice(_sent, device, voice, false);
            send(_sent);
        ...
        void transmit_bank_read_voice(const integral<std::uint8_t, 0, 15> device, const integral<std::uint8_t, 0, 31> voice, patch& data)
        {
            ...
            send(_sent);
            encode_button_voice(_sent, device, 0, true);
            send(_sent);
            encode_button_voice(_sent, device, 0, false);
            send(_sent);
            for (std::size_t _index = 0; _index < 3; ++_index) {
                encode_button_voice(_sent, device, 7, true);
                send(_sent);
                encode_button_voice(_sent, device, 7, false);
                send(_sent);
            }
